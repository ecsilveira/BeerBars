...
/**
 * Classe Reponsavel pelas respostas dos Controllers Monta as saidas dos Filters também
 * 
 * @author B35579
 */
public class WrapResponse {
     ...
     * This pattern matches "at package.class.method(source_code:123)" Note that "source_code" can be a java, scala or js file!
     */
    ...
    private List<String> tryToExtractTheStackTrace(String error) {
        ...
        while (traceMatcher.find()) {
            ...
            stackTrace.add(new StringBuilder().append(className).append(".").append(methodName).append("(").append(sourceFile).append(":")
                    .append(lineNum).append(")").toString());
        }
        return stackTrace;
    }
    ...
    private SimpleResult onCustomCode(HttpMessagesEnum customCode, Context ctx, String data) throws Exception {
        ...
        if (customCode.getType().equals("error")) {
            ...
            StringBuilder toReturn = new StringBuilder("{\"http_code\":").append(customCode.getHttpCode()).append(",\"server_code\":")
                    .append(String.valueOf(customCode.getServerCode())).append(",").append(prepareOK(ctx, data)).append("}");
...
//        response.setHeader("Date", DateTime.now().toString());
    ...
    public SimpleResult wrap(Context ctx, F.Promise<SimpleResult> simpleResult) throws Throwable {
        ...
        ServerLogger.debug("WrapResponse.wrap - Result type:" + result.getWrappedResult().getClass().getName() + " Response Content-Type:"
                + ctx.response().getHeaders().get("Content-Type"));
